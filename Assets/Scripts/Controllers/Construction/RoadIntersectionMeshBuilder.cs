...
namespace Views.Road
{
    ...
    public class RoadIntersectionGenerator
    {
        ...
        public void GenerateIntersection(Vector3 position)
        {
            ...
            {
                var mesh = GenerateMesh(node.Data, node.Neighbors.Where(n => n.NodeType == NodeType.Road).Select(n => n.Data), constructionConfig.RoadWidth);

                var go = new GameObject($"IntersectionMesh {node.Id}");
        ...
        private Mesh GenerateMesh(Vector3 center, IEnumerable<Vector3> neighbors, float roadWidth)
        {
            ...
            var outletId = 0;
            var distanceFromCenter = constructionConfig.SegmentSpacing;
